...
public class EnemyActor extends Actor {
    ...
    public EnemyActor(float width, float height, Enemy enemy) {
        ...
        moveDisplay = new MoveDisplay(enemy);
        moveDisplay.setPosition(getX() + getWidth() / 2 + MoveDisplay.width / 2f, getY() + getHeight());
...
class HealthBar extends Actor {
    ...
    private final Sprite armorIcon;
...
class MoveDisplay extends Actor {
    ...
    public MoveDisplay(Enemy enemy) {
        this.enemy = enemy;
        setWidth(width);
        setHeight(height);
    ...
    public static void setShowMoreInfo(boolean showMoreInfo) {
        MoveDisplay.showMoreInfo = showMoreInfo;
    }
    ...
    public void setMove(Action action) {
        ...
        switch (action.getMove()) {
            case DEFEND:
                move = new Sprite(defend);
                tooltip.setText("Захист", "Ворог планує піти в захист");
                break;
            case ATTACK:
                move = new Sprite(attack);
                tooltip.setText("Атака", "Ворог планує атакувати!");
                break;
            case EFFECTED_ATTACK:
                move = new Sprite(effected_attack);
                if (showMoreInfo && action.isWithEffect)
                    tooltip.setText("Заклинальний удар", "Ворог планує атакувати та накласти " + Arrays.toString(action.effects));
                else
                    tooltip.setText("Заклинальний удар", "Ворог планує атакувати та накласти негативний ефект!");
                break;
            case BUFF:
                move = new Sprite(buff);
                if (showMoreInfo && action.isWithEffect)
                    tooltip.setText("Чари", "Ворог планує накласти на себе " + Arrays.toString(action.effects));
                else
                    tooltip.setText("Чари", "Ворог планує накласти на себе позитивний ефект");
                break;
            case DEBUFF:
                move = new Sprite(debuff);
                if (showMoreInfo && action.isWithEffect)
                    tooltip.setText("Заклинання", "Ворог планує накласти на вас " + Arrays.toString(action.effects));
                else
                    tooltip.setText("Заклинання", "Ворог планує накласти на вас негативний ефект!");
                break;
            case IMPACT:
                move = new Sprite(impact);
                tooltip.setText("Невідома дія", "Ворог замислив щось невідоме...");
                break;
            case BUFFED_ATTACK:
                move = new Sprite(buffed_attack);
                if (showMoreInfo && action.isWithEffect)
                    tooltip.setText("Чарівна атака", "Ворог планує атакувати та накласти на себе " + Arrays.toString(action.effects));
                else
                    tooltip.setText("Чарівна атака", "Ворог планує атакувати та накласти на себе чари!");
                break;
            case BUFFED_DEFENSE:
                move = new Sprite(buffed_defend);
                if (showMoreInfo && action.isWithEffect)
                    tooltip.setText("Чарівний захист", "Ворог планує піти в захист та накласти на себе " + Arrays.toString(action.effects));
                else
